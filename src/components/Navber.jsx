...
const CoustomLinkMobile = ({href,title,className=""})=>{

    const router = useRouter()
 

    return(
        <Link href={href} className={`${className} group relative text-light dark:text-dark`}>
            {title}
...
const Navber = () => {
  ...
  return (
    <header className='w-full px-4 md:px-16 lg:px-32 z-50 lg:py-8 p-0 font-medium flex  dark:text-light items-center justify-between relative'>
...

        {
            ...
            (
                ...
                <nav className='flex items-center justify-center flex-col gap-3'>
                    <CoustomLinkMobile href={"/"} title={"Home"} className=''/>
                    <CoustomLinkMobile href={"/about"} title={"About"} className=''/>
                    <CoustomLinkMobile href={"/projects"} title={"Projects"} className=''/>
                    <CoustomLinkMobile href={"/articles"} title={"Articles"} className='mb-3'/>
                </nav>
                <nav className='flex items-center'>
                    <motion.a href="/" target={"_blank"} whileHover={{y:-2}} whileTap={{scale:0.9}} className='w-6 mr-3'><TwitterIcon/></motion.a>
        ...
        <div className='w-24 h-auto flex items-center justify-center relative '>
